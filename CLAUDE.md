...
- Events have no detail page (too little content per item to justify a URL). `EventCard` renders the description as markdown inside `components/ExpandableText.tsx`, a client component that clamps it to ~3 lines and shows a "Leer más / Ver menos" toggle only when the text actually overflows; the events grid uses `items-start` so an expanded card doesn't stretch its row
