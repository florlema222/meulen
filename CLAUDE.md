...
- **Optional content translation**: markdown frontmatter holds nested `en:` / `pt:` objects (e.g. `en: { title, summary }`). Data loaders take a `locale` arg and call `pickLocalized`, which returns the translated field or falls back to the Spanish base field. Body text is not translated: news cards expand to the Spanish markdown body on every locale; everything else the homepage renders comes from frontmatter.
...
- Events and news have no detail pages (too little content per item to justify a URL). `EventCard` (description) and `NewsCard` (body) render CMS markdown through `components/Markdown.tsx` (links open in a new tab) inside `components/ExpandableText.tsx`, a client component that clamps it to ~3 lines, shows a "Leer más / Ver menos" toggle only when the text actually overflows, and scrolls the text back into view on collapse; both grids use `items-start` so an expanded card doesn't stretch its row
